...
- Java 开发工程师出身，但不执着于开发语言，愿意根据产品机会选择最合适的技术栈
- 独立开发者，目标是发现可以快速验证、快速上线、可持续迭代的热门应用方向
- 重点关注能做成真实产品的项目：SaaS、小工具、AI 应用、浏览器插件、自动化服务、内容/营销工具、个人效率工具、垂直行业应用
- 优先选择有明确用户痛点、商业化潜力、差异化空间、低成本 MVP 可行性的项目
- 喜欢「能直接启发选题」的项目，而不只是技术很强的底层框架、算法库、基础设施或纯学术项目
- 希望每个推荐都能帮助判断：它解决什么问题、面向什么用户、我能否基于它做一个更小但可售卖的产品
...
#### 🎨 创意/好玩类（排最前面，2-3 个）
不是为了有用，而是有趣、好看、有新意——游戏、艺术、新奇实验等。每段写清楚「有意思在哪」，以及能启发什么轻量产品方向。

#### 🛠 工具类（2-3 个）
能直接用的东西——文件处理、自动化、效率提升、开发工具等。每段写清楚「解决了什么问题」，以及适合面向哪类用户做成小产品。

#### 🤖 AI 应用类（2-3 个）
跟大模型相关的——AI 助手、绘图、语音、翻译等。每段写清楚「用 AI 做了什么、普通人怎么用」，以及有没有可验证的商业化切入点。
...
7. 每个项目尽量补一句「独立开发者启发」，说明它可以启发什么 MVP、垂直场景或可售卖小产品
